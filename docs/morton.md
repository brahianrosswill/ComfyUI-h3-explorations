...
| `h3_capture.py` | Captures real q/k/v from a live forward. **First run 2026-08-16**, after sitting unrun since it was written; it produced the activation measurement below that settled link 5. Captures persist under `$H3_CAPTURE_ROOT/` but are not permanent -- the `2026-08-15_dense_124f_1344x768` one that produced the measurement below has since been removed, so that result cannot be re-derived without a new render. Arm it with `H3_CAPTURE` in the environment **before** ComfyUI starts -- it is read at module import, so there is no way to arm it on a running server. |
