...
| **the fl2va and ref2va time curves differ by "7.8% relative"**, as stated in `bench/convert_pdd_lora.py`'s docstring and `docs/h3_pdd.md` | **UNSUPPORTED, marked 2026-09-03.** The figure dates from a 2026-08-26 comparison that no dated record under `bench/results/` carries, so it has no conditions and no origin a reader can run. The direction stands (`h3_pdd.silu_temb_grid` is partition-specific, which is why the converter derives it from `--base`); the size is a copy with no home. `docs/pdd_artifacts.md` points here instead of repeating it | a record from a script that compares the two partitions' `silu(time_embedder(t))` grids over the 1025-row time grid, or delete the figure from the two places that state it |
| **block 49 "carries about four times any other block's error with every block routed on both builds", and "no `dense_blocks` list touches it"**, as first stated in `CHANGELOG.md` 0.99.27 for the Base16 capture records | **Withdrawn 2026-09-03, the same day, on Codex's review.** True only on the block-max build and only under the norm-weighted whole-tensor relative L2; per row block 49 is barely above block 40, and on the running-max build block 40 is the worse one (`bench/results/2026-09-03_sol_exact_base16_capture_{d25f2e8,24908e1}.json`, `exact_all_routed.rel_l2` against `rel_l2_row_mean`). And a `dense_blocks` entry sends the block to sage, bypassing Sol's exact stage entirely, so it does touch it. What stands: a large exact-stage hotspot on the block-max build under the whole-tensor metric, in a few high-norm rows | an all-50-block Sol-versus-sage record on the production trajectory, then a candidate set tested as a set |
| **the Base16 records' `tau_1.0` arm is "the shipped tau"**, and (briefly) "an upper bound on the shipped call's error" | **Withdrawn 2026-09-03.** The node passes sink ranges from the segment table and a capture taken with Sol absent has no table, so the arm is the shipped tau without the shipped call; renamed `tau_1.0_no_sinks`. The "upper bound" that replaced the first label lasted an hour: forced-pair counts are monotone in the sink ranges, relative L2 and cosine are not, because errors can cancel. It is an unsunk diagnostic | a capture whose records carry the segment table (a neutral layout-publication helper armed by Sol or `h3_capture.py`), then the arm re-run with the node's own sink ranges |
| **any reading of the 2026-09-04 ladder verdict as settled: "PDD8 is visibly worse", "sage and Sol are indistinguishable from dense", or "fp16 sage helps"** ([`bench/results/2026-09-04_ladder_2026-09-03_verdict.json`](../bench/results/2026-09-04_ladder_2026-09-03_verdict.json)) | **Not withdrawn; bounded from the day it was written.** One seed per arm, so each contest is one stack judged once. Within a scene every contest shares one clip per rung, so a rung that lost twice in a scene lost on one clip and one defect seen from two stacks. The singles were not scored, so nothing audible was judged and the manifest's audio questions stand open; and stacked pairs were silent by the stacker's design in every session before `3637f6d` (2026-09-05), so no pair verdict in this or any earlier session judged anything audible, whatever the singles carried. **And the PDD8 rung is confounded**, as the owner said on reading the result: every rung above dense carries sage, the three Sol rungs carry Sol, and pdd8 carries the PDD-specific Sol `end_percent` besides; no arm renders PDD8 without Sol or without sage, so its losses attribute to nothing narrower than "the shipped PDD8 graph". What the record does give: the owner's note on every pair resolved through the key, and a direction per scene. The one result consistent across all five scenes is that the shipped PDD8 graph lost to the dense baseline on a named defect each time, and the owner named it as PDD unprompted on two of those pairs while blind. The audio half is the owner's report in conversation, kept in the record's `owner_report_after_join` with that provenance (good across the board, a bit quieter on the PDD clips), and [`bench/results/2026-09-04_ladder_audio_loudness.json`](../bench/results/2026-09-04_ladder_audio_loudness.json) is the per-clip level beside it, one clip per arm, descriptive | PDD8 under stock attention and under sage alone on the same scenes and seed, so the shipped pdd8 clips become one rung of a three-rung PDD ladder with its own baseline (`workflows/build_workflows.py`, the `dense_attn` extra); then a second seed on the same scenes and rungs (the 2026-09-03 session postmortem, forward item 4), and the singles scored through the page for the audio half |
| **"PDD8 is worse than sage at sixteen steps" or "Sol on two of eight steps is free" read as settled from the PDD ladder verdict** ([`bench/results/2026-09-05_pdd_ladder_2026-09-04_verdict.json`](../bench/results/2026-09-05_pdd_ladder_2026-09-04_verdict.json)) | **Bounded, 2026-09-05.** One seed, five scenes, pairs only; the singles were not scored, so audio rests on the owner's pair notes ("louder" for the floor on every compared scene) and on `bench/results/2026-09-04_pdd_ladder_audio_loudness.json`, which is descriptive. What the record does support directionally: the floor won all five against PDD8 under sage alone and four with one can't-tell against the narrow window, so the PDD8 loss attributes to PDD itself (schedule or merge, undecided until `pdd_bake_2026-09-05` is scored) and not to Sol; Sol on two of eight steps read same as sage alone on all five; the shipped four-step window lost on two (standoff, subway). Two of the named defects (the diner "menu across the wall", the subway artifacts) appeared on every PDD arm of their scene and on neither floor clip. |
| **"the bake fixed PDD8" or "the bake changed nothing"**, read from the bake pair verdict ([`bench/results/2026-09-05_pdd_bake_2026-09-05_verdict.json`](../bench/results/2026-09-05_pdd_bake_2026-09-05_verdict.json)) | **Bounded, 2026-09-05.** One seed, five scenes, pairs only, one verdict corrected by the owner after the join, exported value kept beside it in the file. Directionally: the baked arm lost to the sage floor on four scenes and read same on one, on the properties the merged arm lost on the day before (level, lighting, framing, the subway sign), so the PDD8 defects attribute to the eight-step schedule and not to the requantised merge at this seed. Merged against baked read same on two, merged won two and baked won one on composition, so the merge noise's visible effect is not larger than one sample's variation. The bake's stored-weight case (`2026-08-31_bake_realisation.json`) is untouched by this; what it did not deliver is a visible quality change. |
| **"Sol without sage is worse than the sage floor" as settled**, read from the just-Sol verdict ([`bench/results/2026-09-05_sol_nosage_2026-09-04_verdict.json`](../bench/results/2026-09-05_sol_nosage_2026-09-04_verdict.json)) | **Bounded, 2026-09-05.** One seed, five scenes, pairs only, singles unscored. The floor won four with one can't-tell and Sol as shipped won three with two same, but the notes name composition on all but one pair (framing, the orchestra behind the singer, the diner menu, the suspect behind the chaser), which the owner then ruled count as prompt adherence (2026-09-05 evening; the record was first read as sample divergence and says so); the one rendering-quality loss is stairwell (lighting, skin fidelity, level). Direction against just-Sol on adherence at one seed; the shipped chain stays the leader on the 2026-09-03 ladder verdict and on this. |
| **"the encoder pathway carries identity on its own" or "reference rows are just a label"**, read from the 2026-09-04 reference pathway verdict ([`bench/results/2026-09-04_ref_pathway_2026-09-03_verdict.json`](../bench/results/2026-09-04_ref_pathway_2026-09-03_verdict.json)) | **Not withdrawn; bounded from the day it was written.** One seed, pairs only, no single heard. The owner could not tell the both-pathway arm from the encoder-only arm on either conditioner and named no identity item as differing; that is a "can't tell" at one seed, which bounds the effect as not large on this scene and says nothing about its sign. The one thing named, static background people on every encoder-only arm, is a signature at one seed and not a mechanism. The two encoder-only arms read as the same and are different renders ([`bench/results/2026-09-04_ref_pathway_encoder_arms_pixels.json`](../bench/results/2026-09-04_ref_pathway_encoder_arms_pixels.json)) | a second seed on the same five arms (the manifest's `run` line), the singles scored, and the static-people signature either recurring or not |
| **Sol and the shipped sage fallback were compared "on one footing"** via [`2026-09-03_sage_on_base16_capture.json`](../bench/results/2026-09-03_sage_on_base16_capture.json) | **Withdrawn 2026-09-03.** That record scored 512 sampled rows against float64; the Sol records scored every row against fp32. Same statistic name, different sample and reference. The Sol records were regenerated with `sage_<mode>` arms on the same q/k/v, every row, the same reference, and the sage arms agree across both kernel builds; the direction they give is in `docs/research/2026-09-03_sol_exact_pquant_and_base_capture.md`'s addendum. The separate sage record stays as what it was | nothing further: the matched arms are the comparison to cite |
...
PHRASE: no `dense_blocks` list touches it
ALLOW: docs/evidence.md CHANGELOG.md
WHY: withdrawn 2026-09-03; CHANGELOG 0.99.27 quotes it inside its own
     retraction, the research record restates the claim in other words.

PHRASE: an upper bound on the shipped call
ALLOW: docs/evidence.md
WHY: the label lived an hour on 2026-09-03 and was replaced everywhere by
