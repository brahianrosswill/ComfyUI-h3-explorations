...
def case_graph_shift_matches_file():
    ...
    **This is the gap the rewiring opened, and it is real.** The PDD node sits
    ...
    # disagrees, which `problems` above carries. A tree with NO PDD graph is a
    # different state -- discovery found nothing, and no LoRA's presence
    # could change that -- so that one stays red.
    assert graded or skipped, (
        "no PDD graph with a readable step count was found; the LoRA-absent "
        "skip does not cover a tree with nothing to compare")
    if not graded:
...
# `pdd end_percent keeps last step dense` was retired 2026-09-11. It asserted
# that a PDD graph's final evaluation fell outside Sol's band. Since then
# `end_percent` is 1.0 on every graph, adopting upstream (sglang's `sol_attn`
# and core's `BlockSparseAttention` both run Sol through the last step), so
# the property is deliberately false. `git log -S` on the case name finds it.
...
# `ui and api agree` was retired 2026-09-14. It compared the UI form's
# positional read of the PDD node's `steps` against the API form's named one,
# which guarded a widget index. The shipped graphs are API form only, so there
# is no positional read left to drift. `git log -S` on the case name finds it.
